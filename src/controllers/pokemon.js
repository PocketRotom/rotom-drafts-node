...
const unavailablePokemon = require('../../unavailable.json');
...
module.exports = {
    ...
    try {
      ...
      if (error.code == "ER_DUP_ENTRY") {
        return res.status(500).json({
          success: false,
          error:
            "Pokémon Already drafted this draft or you drafted it already... Or I f***ed up",
        });
      }
      return res.status(500).json({
        success: false,
  ...
  addPokemon: async (req, res) => {
    try {
      ...
      let pokemon = await Pokemon.addPokemon(
        name,
        type1,
        type2,
        hp,
    ...
    try {
      ...
      let filteredPokemons = pokemons
        .filter((pokemon) => {
          ...
          if (name.includes("-Gmax") || name.includes("-Mega")) {
            return false;
          }
          if (unavailablePokemon.includes(name)) {
            return false;
          }
          if (type2) {
            return allowedTypes.includes(type1) && allowedTypes.includes(type2);
          } else {
            return allowedTypes.includes(type1);
          }
        })
        .map((pokemon) => {
          return {
  ...
  getProfCupZombie: async (req, res) => {
    try {
      //let leagueID = req.query.leagueID;
  ...
  getProfCupSpider: async (req, res) => {
    try {
      //let leagueID = req.query.leagueID;
